...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const alreadyTrackedToday = localStorage.getItem("lastLocationTrackDate");

    // Force run check-in if there are absolutely no records logged for the account yet on this base
    if (alreadyTrackedToday === today && records.length > 0) {
      return;
    }
    ...
    const runTracking = async () => {
      try {
        ...
        localStorage.setItem("lastLocationTrackDate", today);
      } catch (error) {
  ...
  const handleExportData = () => {
    ...
    XXLSX.utils.book_append_sheet(workbook, worksheet, "Travel History Logs");
    ...
    XXLSX.writeFile(workbook, `Residency_Audit_Report_${homeCountryName}.xlsx`);
  };
