...
export default function TravelTable({ records, onDelete, onEdit }) {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden w-full isolate">
      {/* FIXED: Applied strict overscroll-x-contain and momentum scrolling with layout separation parameters */}
      <div className="w-full overflow-x-auto overflow-y-auto max-h-[600px] overscroll-x-contain [webkit-overflow-scrolling:touch] scrollbar-thin scrollbar-thumb-slate-200 active:scrollbar-thumb-slate-300">
        <table className="w-full border-collapse text-left text-base min-w-[700px] table-auto assignment-matrix">
          <thead className="bg-slate-50 border-b border-slate-200 text-slate-400 text-[10px] uppercase font-bold tracking-wider sticky top-0 z-20 shadow-[0_1px_0_0_rgba(226,232,240,1)]">
            <tr>
              {/* FIXED: Switched to 100% solid bg-slate-50 wrapper with a sharp border split line indicator to handle Safari swipe physics */}
              <th className="px-6 py-3.5 sticky left-0 bg-slate-50 z-30 shadow-[1px_0_0_0_rgba(226,232,240,1)]">
          ...
          <tbody className="divide-y divide-slate-100 text-slate-700 font-medium text-xs">
            {paginatedRecords.map((r) => (
              ...
              >
                {/* FIXED: Enforced a 100% solid bg-white layer (removed white/90 blur) and attached a right shadow divider via custom shadow mapping to clip overlapping data cells */}
                <td className="px-6 py-4 text-slate-900 font-bold sticky left-0 bg-white z-10 whitespace-nowrap shadow-[1px_0_0_0_rgba(241,245,249,1),4px_0_12px_-4px_rgba(0,0,0,0.08)]">
