...
export default function TravelHistory() {
  ...
  // Layout segment control states
  const [activeSection, setActiveSection] = useState("app"); // "app" or "manual"
  const [manualViewType, setManualViewType] = useState("calendar"); // "calendar" or "form"
    ...
    return (
      <div className="h-[70vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-slate-500 text-sm">Loading travel records...</p>
        </div>
      </div>
    );
  }
  ...
  const calendarHomeDays = hasValidTravelRecords ? (calculation?.homeDays ?? 0) : 0;
  const calendarAbroadDays = hasValidTravelRecords ? (calculation?.outsideDays ?? 0) : 0;
      ...
      try {
        ...
        spanDays.forEach((day) => {
          ...
          if (
            ...
            appDatesSet.add(dateStr);
          } else if (record.purpose === "Initial Home Stay" || record.purpose === "Automated System Entry") {
            baseDatesSet.add(dateStr);
          } else {
            manualDatesSet.add(dateStr);
          }
  ...
  return (
    <div className="space-y-2.5 relative text-left w-full">
      {/* Title Header Section */}
      {/* Title Header */}
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-1 border-b border-slate-100 pb-2">
        <div>
          <h1 className="text-xl font-black text-slate-900 tracking-tight">
            Travel History
          </h1>
          <p className="mt-0.5 text-[11px] font-semibold text-blue-800 leading-none">
            Review your travel timeline and residency records.
          </p>
        </div>
      </div>

      {/* Main Mode Toggles Selector Section */}
      ...
      {/* Dynamic Counter KPI Matrix Cards Grid */}
      ...
      {/* Manual Mode Inner Sub-Options Selection Wrapper */}
      {activeSection === "manual" && (
        <div className="space-y-6">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => {
                  setManualViewType("calendar");
                  setShowForm(false);
                }}
                className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all shadow-sm cursor-pointer border ${
                  manualViewType === "calendar"
                    ? "bg-blue-600 text-white border-blue-600"
                    : "bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
                }`}
              >
                Update via Calendar View
              </button>
              <button
                type="button"
                onClick={() => {
          ...
          {/* Inline Form View Block Injection */}
          {manualViewType === "form" && showForm && (
            <TravelForm
          ...
          {/* Integrated Inline Stay Calendar Render Layout */}
          {manualViewType === "calendar" && (
            <StayCalendar
      ...
      {/* Bottom Layout Unified Table View - Only visible when App Tracking is active */}
      {/* NEW CODE BLOCK REPLACEMENT */}
{activeSection === "app" && (
  <div className="w-full bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden mt-3">
    <div className="p-3 border-b border-slate-200 flex items-center justify-between gap-3 bg-slate-50/50">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <h2 className="text-xs sm:text-sm font-black text-slate-900 uppercase tracking-wide truncate">
            Travel Records
          </h2>
          <span className="px-1.5 py-0.5 rounded bg-blue-50 border border-blue-100 text-blue-700 text-[10px] font-extrabold shrink-0">
            {records.length} Logs
          </span>
        </div>
        <p className="text-[10px] text-slate-500 font-medium leading-none mt-0.5 truncate hidden sm:block">
          Your complete travel history logs.
        </p>
      </div>
      ...
      {records.length > 0 && (
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg shadow-sm text-[11px] font-bold shrink-0 cursor-pointer"
        >
...
//                 type="button"
//                 onClick={handleExportData}
//                 className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-xl shadow-md text-xs sm:text-sm font-semibold transition-all duration-300 cursor-pointer"
//               >
...
//               <div className="p-3 sm:p-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
//                 <div>
...
// //   // New Tab layout segment control states
// //   const [activeSection, setActiveSection] = useState("app"); // "app" or "manual"
// //   const [manualViewType, setManualViewType] = useState("calendar"); // "calendar" or "form"
...
// //           <p className="text-sm font-medium text-slate-700">Choose Manual Entry Mode:</p>
// //           <div className="flex gap-3">
// //             <button
// //               type="button"
// //               onClick={() => {
...
// //           {/* Inline Form View Block Injection */}
// //           {manualViewType === "form" && showForm && (
// //             // <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 mt-2 shadow-sm transition-all">
// //               <TravelForm
// //                 initialData={editingRecord}
// //                 travelRecords={records}
// //                 onSubmit={handleSaveRecord}
...
// //                   <FiCalendar className="text-blue-500" /> Integrated Stay Calendar View
// //                 </h3>
// //                 <div className="flex flex-wrap gap-2">
// //                   <span className="px-2.5 py-1 text-xs font-semibold bg-green-50 text-green-700 rounded-lg">🏠 Home: {calendarHomeDays} Days</span>
// //                   <span className="px-2.5 py-1 text-xs font-semibold bg-blue-50 text-blue-700 rounded-lg">🌍 Abroad: {calendarAbroadDays} Days</span>
// //                 </div>
// //               </div>
// //               <StayCalendar
// //                 dayMap={computedDayMap}
